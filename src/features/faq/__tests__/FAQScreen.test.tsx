...
describe('FAQScreen', () => {
  ...
  it('shows FAQ content when sections are expanded', () => {
    ...
    expect(screen.getByText('If I complete an Exhibition Clash as part of a story mission, does that replace the Clash at the 8th hour, or do I have to resolve two Clashes?')).toBeTruthy();
    ...
    expect(screen.getByText('How does Monster facing work? Do Monsters always end their movement facing the target of their movement action? Is this true also for non-attack oriented movement actions? Like the Firstmen Warriors running to the side of the White Ape Troll?')).toBeTruthy();
  });
  ...
  it('toggles section expansion when header is pressed multiple times', () => {
    ...
    // Wait a moment for the state to update, then check content
    expect(screen.getByText('It replaces the Clash at 8th hour.')).toBeTruthy();

    // Second press should collapse (content should still be visible since we're just testing toggle)
    fireEvent.press(explorationHeader);
  ...
  it('shows correct number of FAQ items', () => {
    ...
    const explorationItems = explorationSection.findAllByType('View').filter((view: any) => {
      ...
      return view.props.children && 
             ...
             view.props.children.some((child: any) => 
