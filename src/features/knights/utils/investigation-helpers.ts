...
export const getInvestigationSuccessRate = (
    ...
    return {
        normal: normalInvestigations.length > 0 ? (normalSuccess / normalInvestigations.length) * 100 : 0,
        lead: leadInvestigations.length > 0 ? (leadSuccess / leadInvestigations.length) * 100 : 0,
        overall: investigations.length > 0 ? ((normalSuccess + leadSuccess) / investigations.length) * 100 : 0,
    };
};
