...
describe('AddExistingKnights', () => {
  ...
  it('renders empty state when no knights available', () => {
    const { getByText } = render(
      <AddExistingKnights list={[]} onAddActive={mockOnAddActive} onBench={mockOnBench} />
    );
  ...
  it('renders knight list when knights are available', () => {
    ...
    const { getByText } = render(
      <AddExistingKnights list={mockList} onAddActive={mockOnAddActive} onBench={mockOnBench} />
    );
  ...
  it('renders action buttons for each knight', () => {
    const mockList = [{ knightUID: 'knight-1', name: 'Sir Galahad', catalogId: 'galahad' }];
  ...
  it('handles long knight names with numberOfLines', () => {
    const mockList = [
      {
        knightUID: 'knight-1',
        name: 'Very Long Knight Name That Should Be Truncated',
        catalogId: 'galahad',
      },
    ];
    ...
    const mockList = [
      {
        knightUID: 'knight-1',
        name: 'Sir Galahad',
        catalogId: 'very-long-catalog-id-that-should-be-truncated',
      },
    ];
