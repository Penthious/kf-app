import type {
  MockCardProps,
  MockInvestigationPillsProps,
  MockInvestigationChooserProps,
...
jest.mock('../InvestigationChooser', () => {
  ...
  function MockInvestigationChooser({
    code,
    locked,
    onNormalPass,
    onNormalFail,
...
describe('ChapterInvestigations', () => {
  ...
  it('renders the component with chapter header', () => {
    const { getByText } = render(<ChapterInvestigations knightUID='knight-1' chapter={3} />);
  ...
  it('renders investigation pills', () => {
    const { getByTestId } = render(<ChapterInvestigations knightUID='knight-1' chapter={3} />);

    expect(getByTestId('investigation-pills')).toBeTruthy();
  });

  it('handles different chapter numbers', () => {
    const { getByText } = render(<ChapterInvestigations knightUID='knight-1' chapter={5} />);

    expect(getByText('Chapter 5 • Investigations')).toBeTruthy();
  });

  it('handles chapter 0 by defaulting to chapter 1', () => {
    const { getByText } = render(<ChapterInvestigations knightUID='knight-1' chapter={0} />);

    expect(getByText('Chapter 0 • Investigations')).toBeTruthy();
  });

  it('handles negative chapter numbers', () => {
    const { getByText } = render(<ChapterInvestigations knightUID='knight-1' chapter={-1} />);

    expect(getByText('Chapter -1 • Investigations')).toBeTruthy();
  });

  it('handles large chapter numbers', () => {
    const { getByText } = render(<ChapterInvestigations knightUID='knight-1' chapter={10} />);
