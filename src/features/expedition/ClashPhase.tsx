...
export default function ClashPhase({ campaignId }: ClashPhaseProps) {
  ...
  const { campaigns, completeClash, scavengeCards } = useCampaigns();
  ...
  const isExhibitionClash = expedition.currentPhase === 'clash';
  const clashType = isExhibitionClash ? 'exhibition' : 'full';

  const handleCompleteClash = (outcome: 'victory' | 'defeat') => {
    Alert.alert('Complete Clash', `Mark clash as ${outcome}?`, [
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.phaseDescription}>
        {isExhibitionClash
          ? 'A smaller, easier tactical battle at the midpoint of the expedition.'
          : 'The final decisive battle of the expedition.'}
      </Text>

      <Card>
        <View style={styles.clashInfo}>
          <Text style={styles.clashType}>{clashType} Clash</Text>
          <Text style={styles.clashDescription}>
            {isExhibitionClash
              ? 'Start with Generic Single Wound cards in the Wound Stack. Shorter, easier fight.'
              : 'Full tactical battle with all mechanics. Last until either side is defeated.'}
          </Text>
        </View>
      ...
      {clashResults.length > 0 && (
        <Card>
          <Text style={styles.sectionTitle}>Previous Clash Results</Text>
      ...
      <ScavengeSelectionModal
        ...
        availableCards={SCAVENGE_DECK.filter(card =>
          getAvailableScavengeTypes(isExhibitionClash ? 'exhibition-clash' : 'full-clash').includes(
            card.type
          )
        )}
      />
