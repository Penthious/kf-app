  ...
  return ({
    ...
    <View testID={testID}>
      ...
      <View testID={testID ? `${testID}-display` : undefined}>
        <Text testID={testID ? `${testID}-value` : undefined}>{String(value ?? 0)}</Text>
      </View>
      <Pressable
...
const kingdom: KingdomView = {
  ...
  adventures: [
    {
      id: 'principality-of-stone:stone-puzzle',
      name: 'Stone Puzzle',
      singleAttempt: true,
      roll: { min: 2, max: 10 },
      completedCount: 0,
    ...
    {
      id: 'principality-of-stone:scavenge-supplies',
...
describe('AdventuresCard', () => {
  ...
  it('renders single attempt adventures with pill buttons', () => {
    ...
    expect(getByTestId('adventure-name-principality-of-stone:stone-puzzle').props.children).toBe(
      'Stone Puzzle'
    );

    // Check roll information
    expect(getByTestId('adventure-roll-principality-of-stone:stone-puzzle').props.children).toEqual(
      ['Roll: ', 2, '–', 10]
    );
  ...
  it('renders multi-attempt adventures with steppers', () => {
    ...
    // Check adventure name
    expect(
      getByTestId('adventure-name-principality-of-stone:scavenge-supplies').props.children
    ).toBe('Scavenge Supplies');

    // Check roll information
    expect(
      getByTestId('adventure-roll-principality-of-stone:scavenge-supplies').props.children
    ).toEqual(['Roll: ', 3, '–', 9]);
  ...
  it('handles multiple adventures correctly', () => {
    ...
    expect(getByTestId('adventure-name-principality-of-stone:first-adventure').props.children).toBe(
      'First Adventure'
    );
    expect(
      getByTestId('adventure-name-principality-of-stone:second-adventure').props.children
    ).toBe('Second Adventure');
    expect(getByTestId('adventure-name-principality-of-stone:third-adventure').props.children).toBe(
      'Third Adventure'
    );
  });
