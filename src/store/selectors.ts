...
export const selectMemberByKnightUID = (campaignId: string, knightUID: string) => (state: CampaignsState) => 
...
export const selectCampaignAdventures = (campaignId: string, kingdomId: string) => (state: CampaignsState) => 
...
export const selectAllKnights = (state: KnightsState) => 
...
export const selectAllMonsters = (state: MonstersState) => 
    state.all;
...
// Combined Selectors
export const selectKnightWithMemberInfo = (campaignId: string, knightUID: string) => 
...
export const selectActiveKnightMembers = (campaignId: string) => 
    (campaignsState: CampaignsState, knightsState: KnightsState) => {
...
export const selectCampaignWithMembers = (campaignId: string) => 
    (campaignsState: CampaignsState, knightsState: KnightsState) => {
        const campaign = selectCampaignById(campaignId)(campaignsState);
        if (!campaign) return undefined;
        
        const membersWithKnights = campaign.members.map(member => ({
            member,
            knight: selectKnightById(member.knightUID)(knightsState),
        })).filter(item => item.knight);
