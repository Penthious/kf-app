...
export type CampaignsActions = {
    ...
    // Member actions
    addKnightToCampaign: (campaignId: string, knightUID: string, meta?: { catalogId: string; displayName: string }) => object | { conflict: string };
    addKnightAsBenched: (campaignId: string, knightUID: string, meta?: { catalogId: string; displayName: string }) => void;
    ...
    // Adventure actions
    setAdventureProgress: (campaignId: string, kingdomId: string, adventureId: string, opts?: { singleAttempt?: boolean; delta?: number }) => void;
};
            ...
            const updatedKingdoms = c.kingdoms.find(k => k.kingdomId === kingdomId)
                ? c.kingdoms.map(k =>
                    k.kingdomId === kingdomId
                        ? { ...k, adventures: updatedAdventures }
                        : k
                )
