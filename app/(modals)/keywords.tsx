  ...
  const data = useMemo(() => {
    ...
    const filtered = catalog
      .filter(k => {
        ...
        const hay =
          `${k.name} ${k.summary} ${k.rulesText} ${(k.tags || []).join(' ')}`.toLowerCase();
        ...
        >
          {sections.map(letter => (
            <View key={`section-${letter}`} style={{ marginBottom: 16 }}>
              <Text style={{ color: tokens.textMuted, marginBottom: 8, fontWeight: '700' }}>
                {letter}
              </Text>
              {data[letter]!.map((k, i) => (
                <Pressable
                ...
                >
                  <Card>
                    ...
                    <Text style={{ color: tokens.textMuted }} numberOfLines={2}>
                      {k.summary}
                    </Text>
                  </Card>
                </Pressable>
        ...
        >
          <SafeAreaView style={{ flex: 1, backgroundColor: tokens.bg }}>
            ...
            >
              ...
              {selected?.stackable ? (
                <Text style={{ color: tokens.textMuted }}>Stackable</Text>
              ) : null}
            </View>
            <ScrollView
            ...
            >
              ...
              {selected?.seeAlso?.length ? (
                <Text style={{ color: tokens.textMuted, marginTop: 12 }}>
                  See also: {selected.seeAlso.join(', ')}
                </Text>
              ) : null}
              {selected?.page ? (
                <Text style={{ color: tokens.textMuted, marginTop: 4 }}>
                  Rulebook p.{selected.page}
                </Text>
              ) : null}
            </ScrollView>
            ...
            <Pressable
              ...
              style={{ margin: 16, padding: 12, borderRadius: 12, backgroundColor: tokens.accent }}
            >
              <Text style={{ textAlign: 'center', color: '#0B0B0B', fontWeight: '800' }}>
                Close
              </Text>
            </Pressable>
          </SafeAreaView>
